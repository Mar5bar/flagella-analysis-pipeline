...
To perform automated waveform analysis, run `batchProcess.m` in MATLAB. You will be prompted to select a directory containing traced images (i.e. the same directory that was processed in the previous step).
...
1. Run `batchProcess.m` using the (now-processed) `example_data` directory. This should take a few seconds on most machines.
